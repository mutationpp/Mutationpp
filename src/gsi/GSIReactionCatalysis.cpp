...
public:
    CatalysisReaction( ARGS l_data_gsi_reaction )
        ...
        parseFormula( l_data_gsi_reaction.s_thermo, *(l_data_gsi_reaction.s_iter_reaction) );

        const Mutation::Utilities::IO::XmlElement& l_node_rate_law = *(l_data_gsi_reaction.s_iter_reaction->begin());
        DataGSIRateLaw l_data_gsi_rate_law = { l_data_gsi_reaction.s_thermo, l_node_rate_law, m_reactants };
        mp_rate_law = Mutation::Utilities::Config::Factory<GSIRateLaw>::create( l_node_rate_law.tag(), l_data_gsi_rate_law );

        if ( mp_rate_law == NULL ) { l_data_gsi_reaction.s_iter_reaction->parseError( "A rate law must be provided for this reaction!" ); }
    }
...
private:
    void parseFormula( const Mutation::Thermodynamics::Thermodynamics& l_thermo, const Mutation::Utilities::IO::XmlElement& l_node_reaction  ){

        std::string l_reactants;
        std::string l_products;
        splitFormulainReactantsProducts( l_reactants, l_products, l_node_reaction );
        
        parseSpecies( m_reactants, l_reactants, l_node_reaction, l_thermo );
        parseSpecies( m_products, l_products, l_node_reaction, l_thermo );

    }

//=============================================================================================================

    void splitFormulainReactantsProducts( std::string& l_reactants, std::string& l_products, const Mutation::Utilities::IO::XmlElement& l_node_reaction ){

        size_t l_pos_equal = m_formula.find( "=" );
        if( l_pos_equal == std::string::npos ){
            l_node_reaction.parseError( ( std::string( "Reaction formula " ) + m_formula + std::string( " does not have '=' or '=>'!" ) ).c_str() );
        } // @todo test
...
//=============================================================================================================
    ...
    void parseSpecies( std::vector<int>& l_species, std::string& l_str_chem_species, const Mutation::Utilities::IO::XmlElement& l_node_reaction, const Mutation::Thermodynamics::Thermodynamics& l_thermo ){
        /** @todo Clean */
        ...
        while ( c != l_str_chem_species.size() ) {
            ...
            if ( add_species ) {
                ...
                if( index == -1 ){
                    l_node_reaction.parseError( ( "Species " + l_str_chem_species.substr( s, e-s+1 ) + " is not in the mixture list or a species in the wall phase!" ).c_str() );
                }
...
Mutation::Utilities::Config::ObjectProvider<CatalysisReaction, GSIReaction> catalysis_reaction("catalysis");
