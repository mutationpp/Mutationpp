...
CatalyticReaction::CatalyticReaction( const IO::XmlElement& node, const class Thermodynamics& thermo, const std::string m_category, const Mutation::gsi::catalysisSurfaceProperties* surf_props )
                                    : m_formula( "" ),
                                      m_conserves( true ),
                                      m_thermo( thermo ),
...
CatalyticReaction::~CatalyticReaction(){
...
void CatalyticReaction::parseFormula(const IO::XmlElement& node, const class Thermodynamics& thermo)
{
...
void CatalyticReaction::parseSpecies(
...
{  
    ...
    while (c != str.size()) {
        ...
        int index;
        if (add_species) {
            index = m_thermo.speciesIndex(str.substr(s,e-s+1));
...
void CatalyticReaction::errorInvalidRateLawGamma( const std::string& l_gamma_rate_law ){
