...
void Kinetics::closeReactions(const bool validate_mechanism) 
{
    ...
    mp_r_22  = new double [m_thermo.nSpecies()];
...
void Kinetics::R(double* const mp_r)
{
    // Sum up contributions from two processes:
    // diss/rec and exchange reactions.
    std::fill(mp_r, mp_r+m_thermo.nSpecies(), 0.0);
    R_22(mp_r_22);
    R_23(mp_r_23);

    for (int i=0; i<m_thermo.nSpecies(); ++i)
        mp_r[i] = mp_r_22[i] + mp_r_23[i];

    // The production terms should be kg/m^3-s then 
    // we need to multiply by the mass particle since 
    // the R terms have dimensions m^-3-s^-1.
    for (int i = 0; i < m_thermo.nSpecies(); ++i)
        mp_r[i] *= m_thermo.speciesMw(i) / NA;
}

//==============================================================================

// Compute the production term R_22 due to exchange reactions
// Ref: Sect. 3.6, pp. 75, Kustova & Nagnibeda, 2009.
void Kinetics::R_22(double* const p_r_22)
{
    double T = m_thermo.T();
    double KBT = KB * T;
    double Tv[3];
    for (int i=0; i<m_thermo.nEnergyEqns()-1; ++i)
...
*/
    ...
    // N2(i) + O <=> NO(k) + N
    // O2(i) + N <=> NO(k) + O
    double k_N2_O[lN2][lNO]; // mol^-1 m^3 s^-1
    double k_O2_N[lO2][lNO]; 

    // TODO: risky to put all these constants here,
    // should be read from 
    // constants for N2,O2
    ...
    // FIXME: why recompute partition funtions here as 
    // already calculate previously (which temperature)
    ...
    double B1_n2[lNO] = {}, B2_n2[lNO] = {}; // m^3 s^-1
    ...
    /* TODO: add/test other simpler models ...
    ////////////////////////////////////////////////////////////////////////////
    ...
    */
    ...
    //double R_exch_22[m_thermo.nSpecies()] = {};
    ...
    p_r_22[2] =    (nd[3] * nd[1] * k_exch_N2_NO - nd[2] * nd[0] * k_exch_NO_N2) + 
	           (nd[4] * nd[0] * k_exch_O2_NO - nd[2] * nd[1] * k_exch_NO_O2);
    // N2
    //R_exch_22[1] = nd[2] * nd[0] * k_exch_NO_N2 - nd[3] * nd[1] * k_exch_N2_NO;
    p_r_22[3] = nd[2] * nd[0] * k_exch_NO_N2 - nd[3] * nd[1] * k_exch_N2_NO;

    // O2
    //R_exch_22[2] = nd[2] * nd[1] * k_exch_NO_O2 - nd[4] * nd[0] * k_exch_O2_NO;
    p_r_22[4] = nd[2] * nd[1] * k_exch_NO_O2 - nd[4] * nd[0] * k_exch_O2_NO;

    std::cout << " R_react_22[i] " << std::endl;
    //for (int i=0; i<m_thermo.nMolecules(); ++i) {
    for (int i=0; i<m_thermo.nSpecies(); ++i) {
        //p_r_22[i] = R_exch_22[i];
	std::cout << " p_r_22["<<i<<"] = " << p_r_22[i] << std::endl;
    }
}
...
{
    const double * p_X = m_thermo.X(); // [-]
    const double   n   = m_thermo.numberDensity(); // [m^-3]
    std::cout << "number density: " << n << std::endl;
    double T = m_thermo.T(); // [K]
    double KBT = KB * T; // [J]
    std::cout << T << std::endl;
    double Tv[3];
    for (int i=0; i<3; ++i) {
    ...
    // Dissociation energy
    const double de[m_thermo.nMolecules()] = {1.0409017238088574e-18,
	1.5636156480913654e-18, 8.196091362099268e-19}; // [J]
    for (int i=0; i<m_thermo.nMolecules(); ++i) {
	std::cout << "de: " << de[i] << std::endl;
    }

    // Partial number densities
    double nd[m_thermo.nSpecies()] = {}; // [m^-3]
    for (int i=0; i<m_thermo.nSpecies(); ++i) {
        nd[i] = p_X[i] * n;
    ...
    double A, nn, Ea = 0.;
    for (int i=0; i<m_thermo.nSpecies(); ++i) { // N O NO N2 O2
    ...
    // Compute STS recombination coefficients, k_rec,c^d(0) [mol^-2 m^6 s^-1]
    std::cout << " Recombination STS rates ... " << std::endl;
    double fac0 = HP * HP * HP * pow(2. * PI * KBT, -1.5);
    ...
    // Compute the summation term in eq. 3.91 or 3.94.
    // NO
    for (int i=0; i<m_thermo.nSpecies(); ++i) {
	fac0 = 0., fac1 = 0.; 
        for (int j=0; j<lNO; ++j) {
    ...
    // NO
    p_r_23[2] =     nd[0] * (nd[0]*nd[1]*kr_no[0]-nd[2]*kd_no[0]) +
    ...
    // N2
    p_r_23[3] =     nd[0] * (nd[0]*nd[0]*kr_n2[0]-nd[3]*kd_n2[0]) +
    ...
    // O2
    p_r_23[4] =     nd[0] * (nd[1]*nd[1]*kr_o2[0]-nd[4]*kd_o2[0]) +
	...
	std::cout <<  " p_r_23["<<i<<"]" << p_r_23[i] << std::endl;
    }
}
