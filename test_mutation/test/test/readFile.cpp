...
bool ReadFile::compareFiles()	{
	bool result;
		...
		string line_file1;
		string line_file2;
		string name_file1("test1.txt");
		string name_file2("test2.txt");
		char *endptr_line1;
		char *endptr_line2;
		double out_double1;
		double out_double2;
		...
		while(file1.good() && file2.good()){
			...
			while(out_double1 == out_double2) {

				cout << cpt++ << setw(5) << " : "    <<  " mutation : " << setw(15) << out_double1 <<  "\t mutation++ : " << out_double2 << endl;

				while (((endptr_line1 != '\0') && (out_double1 = strtod(endptr_line1,&endptr_line1)))
						&& ((endptr_line2 != '\0') && (out_double2 = strtod(endptr_line2,&endptr_line2)))){
						cout << cpt++ << setw(5) << " : "  << " mutation : " << setw(15) << out_double1 <<  "\t mutation++ : " << out_double2 << endl;
