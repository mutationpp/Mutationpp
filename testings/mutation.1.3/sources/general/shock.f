...
C-----------------------------------------------------------------------
     ...
     &                 J(1:3,1:3), D, MM, POSITION, Z(NMAX+3), 
     &                 ZP(NMAX+3), RHO, TINI, TOUT, TMAX,
     &                 DT, RTOL, ATOL, RWORK(LRW), ALPHA, XTOL, 
     &                 RHO2, RHOV2, RHOE2, TV2(1:NMAX),
     &                 RHO1, RHOV1, RHOE1, Y(1:NMAX), TV1(1:NMAX),
     &                 EM1(1:NMAX), EM2(1:NMAX), EM3(1:NMAX),
     &                 EM4(1:NMAX), EM5(1:NMAX), EM6(1:NMAX),
     &                 ND1, ND2, RHOI2(1:NMAX), RHOET
...
C     State 1
      CALL MOLE2MASSFRAC (WR1, LWR1, X, Y)
      CALL NUMBERD (WR1, LWR1, P1, T1, T1, X, ND1)
      CALL DENSITY (WR1, LWR1, X, ND1, RHO1)
      RHOV1 = RHO1 *US
      DO I = 1, NV
        TV1(I) = T1
      ENDDO
      CALL ENERGYMASS (WR1, LWR1, WI, LWI, T1, T1, T1, TV1, P1, EM1,
     &                 EM2, EM3, EM4, EM5, EM6)
      RHOE1 = 0.D0
      DO I = 1, NS
        RHOE1 = RHOE1 +Y(I)*EM1(I)
      ENDDO
      RHOE1 = RHOE1 *RHO1 +0.5D0*RHOV1*RHOV1/RHO1

C     State 2
      CALL NUMBERD (WR1, LWR1, P2, T2, T2, X, ND2)
      CALL DENSITY (WR1, LWR1, X, ND2, RHO2)
      RHOV2 = RHO2 *(US-U2)
      DO I = 1, NV
        TV2(I) = T2
      ENDDO
      CALL ENERGYMASS (WR1, LWR1, WI, LWI, T2, T2, T2, TV2, P2, EM1,
     &                 EM2, EM3, EM4, EM5, EM6)
      RHOE2 = 0.D0
      DO I = 1, NS
        RHOE2 = RHOE2 +Y(I)*EM1(I)
      ENDDO
      RHOE2 = RHOE2 *RHO2 +0.5D0*RHOV2*RHOV2/RHO2

      TINI = T2
      CALL SYSTEMWEI (WR1, LWR1, WI, LWI,  RHO1, RHOV1, RHOE1, P1,
     &                RHO2, RHOV2, RHOE2, TINI, Y, R0)
      CALL JACOBIANWEI (WR1, LWR1, WI, LWI,  RHO1, RHOV1, RHOE1, P1,
     &                  RHO2, RHOV2, RHOE2, T2, Y, J)
...
C     Underrelaxation parameter ALPHA \in ]0, 1]
     ...
     &                  RHO2, RHOV2, RHOE2, TINI, Y, J)
      ...
      DO IS = 1, NS
        RHOI2(IS) = Y(IS) *RHO2
      ENDDO
      RHOET = RHOE2 - 0.5*RHOV2**2/RHO2
      CALL TCNEQNEWTON (WR1, LWR1, WI, LWI, RHOET, RHOI2, TINI, T2)
      CALL MASS2PRESSURE (WR1, LWR1, RHO2, T2, T2, Y, P2)
      ...
      WRITE(*,*) ' -RHO2*(US-U2):        ', RHOV2,'[kg/(m2 s)]'
...
C-----------------------------------------------------------------------
C-----------------------------------------------------------------------
      SUBROUTINE SYSTEMWEI (WR1, LWR1, WI, LWI,  RHO1, RHOV1, RHOE1, P1,
     &                      RHO2, RHOV2, RHOE2, TINI, Y2, R)
C-----------------------------------------------------------------------
C    This subroutines computes the residual for the Rankine-Hugoniot
C    relations in thermal equilibrium and frozen chemistry.
C-----------------------------------------------------------------------
      IMPLICIT NONE
      INCLUDE '../general/memory.cmn'
C-----------------------------------------------------------------------
     ...
     &                 RHOE2, TINI, Y2(1:NS), R(1:3)
C-----------------------------------------------------------------------
      INTEGER IS 
      DOUBLE PRECISION V1, V2, T2, P2, RHOI2(1:NS), RHOET
C-----------------------------------------------------------------------
      V2 = RHOV2 /RHO2
      V1 = RHOV1 /RHO1
      ...
      END SUBROUTINE SYSTEMWEI
C-----------------------------------------------------------------------
C-----------------------------------------------------------------------
      SUBROUTINE JACOBIANWEI (WR1, LWR1, WI, LWI,  RHO1, RHOV1, RHOE1, 
     &                        P1, RHO2, RHOV2, RHOE2, TINI, Y2, J)
C-----------------------------------------------------------------------
C    This subroutines computes the Jacobian matrix for the Rankine-
C    Hugoniot relations in thermal equilibrium and frozen chemistry.
C-----------------------------------------------------------------------
      IMPLICIT NONE
      INCLUDE '../general/memory.cmn'
C-----------------------------------------------------------------------
      INTEGER LWR1, LWI, WI(1:LWI)
      DOUBLE PRECISION WR1(LWR1), RHO1, RHOV1, RHOE1, P1, RHO2, RHOV2, 
     &                 RHOE2, TINI, Y2(1:NS), J(1:3,1:3)
C-----------------------------------------------------------------------
      INTEGER I
      DOUBLE PRECISION R(1:3), R0(1:3), EPS, EPSP1 
C-----------------------------------------------------------------------
      EPS = 1.D-2; EPSP1 = 1.D0 +EPS

      CALL SYSTEMWEI (WR1, LWR1, WI, LWI,  RHO1, RHOV1, RHOE1, P1,
     &                RHO2, RHOV2, RHOE2, TINI, Y2, R0)

      CALL SYSTEMWEI (WR1, LWR1, WI, LWI,  RHO1, RHOV1, RHOE1, P1,
     &                RHO2*EPSP1, RHOV2, RHOE2, TINI, Y2, R)
      DO I = 1, 3
        J(I,1) = (R(I)-R0(I))/(RHO2  *EPS)
      ENDDO
      CALL SYSTEMWEI (WR1, LWR1, WI, LWI,  RHO1, RHOV1, RHOE1, P1,
     &                RHO2, RHOV2*EPSP1, RHOE2, TINI, Y2, R)
      DO I = 1, 3
        J(I,2) = (R(I)-R0(I))/(RHOV2  *EPS)
      ENDDO
      CALL SYSTEMWEI (WR1, LWR1, WI, LWI,  RHO1, RHOV1, RHOE1, P1,
     &                RHO2, RHOV2, RHOE2*EPSP1, TINI, Y2, R)
      DO I = 1, 3
        J(I,3) = (R(I)-R0(I))/(RHOE2  *EPS)
      ENDDO
     
      END SUBROUTINE JACOBIANWEI
C-----------------------------------------------------------------------
C-----------------------------------------------------------------------
